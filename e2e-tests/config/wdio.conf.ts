...
import { getBaseUrl, generateReport, getBrowserName } from './helpers/functions';
...
export const config: Options.Testrunner = {
    ...
    //
    // specs: getFeatureFiles(false, argv.domain, getEnv()),
    // Patterns to exclude.
    exclude: [
    ...
    baseUrl: getBaseUrl(argv.env || "stage"),
    //
    ...
    cucumberOpts: {
        ...
        require: ['./e2e-tests/step-definitions/**/*.steps.ts'],
    ...
    onPrepare: function () {
        try {
            if (fs.existsSync('./reports')) {
                ...
                console.log(`==========================================================================\n`);
                const fileDir = "./.previous-".concat(`${Date.now()}`);
                fs.renameSync('./reports', fileDir);
            }
        } catch (error) {
            console.error('Error during onPrepare hook:', error);
        }
    },
    /**
    ...
    before: async function () {
        ...
        (globalThis as any).__vars[sessionId + ".browserName"] = browserName;
        ...
        if (os.platform() == "darwin") {
            (globalThis as any).__vars[sessionId + ".osx"] = true;
        }
    },
    /**
    ...
    beforeScenario: async function (world, context) {
        ...
        if (skipStep) {
            (globalThis as any).__vars[sessionId + ".skipStep"] = true;
        }
        if (scenarioTag.includes("@multi-sku-order")) {
            (globalThis as any).__vars[sessionId + ".bulkOrder"] = true;
        }
    ...
    /**
     *
     * Runs after a Cucumber Sncenario.
     * @param {ITestCaseHookParameter} world            world object containing information on pickle and test step
     * @param {object}                 result           results object containing scenario results
     * @param {boolean}                result.passed    true if scenario has passed
     * @param {string}                 result.error     error stack if scenario failed
    ...
    afterScenario: async function (world, result, context) {
        const sessionId = browser.sessionId;
        // await browser.reset();
        if ((globalThis as any).__vars[sessionId + ".skipStep"]) {
            delete (globalThis as any).__vars[sessionId + ".skipStep"];
        }
        await commonHelpers.deleteAllBrowserData();
    },
    /**
